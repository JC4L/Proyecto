...
@Component({
  ...
  template: `
    <aside class="flex flex-col h-full w-64 dark:bg-bg-dark bg-bg-light text-bg-dark dark:text-white transition-theme">
      <!-- Brand -->
      <div class="flex items-center gap-3 px-6 border-b dark:border-slate-800 border-bg-dark/10 h-16 bg-white dark:bg-surface-dark transition-theme">
      ...
      <div class="p-4 border-r border-t dark:border-slate-800  border-slate-200 transition-theme">
        <a
          routerLink="/about-us"
          class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-bg-dark/70 dark:text-white/70 hover:bg-bg-dark/10 dark:hover:bg-white/10 hover:text-bg-dark dark:hover:text-white border border-transparent transition-smooth"
        >
