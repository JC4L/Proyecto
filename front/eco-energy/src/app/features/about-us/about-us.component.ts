import { Component } from "@angular/core";
import { ThemeService } from "../../core/services/theme.service";

@Component({
  selector: 'app-about-us',
  template: `
      <div class="animate-fade-in">
        <!-- Hero Section -->
        <section class="mx-auto text-center">
          <div class="">
            <h1 class="text-2xl md:text-3xl font-black text-slate-900 dark:text-slate-100 mb-2 leading-tight">
              Conoce al equipo detrás de <span class="text-primary">EcoEnergy</span>
            </h1>
            <p class="text-lg text-slate-600 dark:text-slate-400">
              Somos un equipo dedicado a construir software que promueva la transición a la energía renovable.
            </p>
          </div>  
        </section>

        <!-- Team Grid -->
        <section class="mx-auto pt-6">
        <div class="flex justify-center gap-8 flex-wrap">
        <!-- Team Member 1 -->
          <article class="bg-white dark:bg-slate-900 border border-primary/10 rounded-xl overflow-hidden shadow-sm hover:shadow-md flex flex-col max-w-70 hover:scale-101 hover:-translate-y-2 transition-all group">
          <div class="w-full h-50 relative">
          <img alt="Cat sospechoso" class="w-full h-full object-cover" data-alt="Professional portrait of a male developer" src="./images/juan-perfil/juan-perfil.jpeg"/>
          <div class="absolute inset-0 bg-linear-to-t from-slate-900/80 to-transparent"></div>
          <div class="absolute bottom-4 left-4">
          <h3 class="text-white text-md font-bold">Juan Álvarez</h3>
          <p class="text-blue-400 font-medium text-xs">E-commerce Technical Support Engineer</p>
          </div>
          </div>
          <div class="p-4 grow">
          <blockquote class="text-slate-600 dark:text-slate-400 italic text-sm border-l-2 border-blue-400 pl-4">
              "La distinción entre el presente, pasado y futuro, es sólo una ilusión obstinadamente persistente." - Albert Einstein
          </blockquote>
          </div>
          <div class="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
          <div class="flex gap-3">
            <a class="text-slate-400 hover:text-primary transition-colors" href="#" target="_blank">
              <svg class="w-7 h-7 fill-current" viewbox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"></path></svg>  
            </a>
            <a class="text-slate-400 hover:text-primary transition-colors" href="#" target="_blank">
              <svg class="w-7 h-7 fill-current" viewbox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"></path></svg>
            </a>
          </div>
          <button class="bg-blue-400 hover:bg-blue-400/90 text-slate-900 px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer">
              Connect
          </button>
          </div>
          </article>

                  <!-- Team Member 2 -->
          <article class="bg-white dark:bg-slate-900 border border-primary/10 rounded-xl overflow-hidden shadow-sm hover:shadow-md flex flex-col max-w-70 hover:scale-101 hover:-translate-y-2 transition-all group">
          <div class="w-full h-50 relative">
          <img alt="Cat sospechoso" class="w-full h-full object-cover" data-alt="Professional portrait of a male developer" src="./images/manuel-perfil/cat-sospechoso.png"/>
          <div class="absolute inset-0 bg-linear-to-t from-slate-900/70 to-transparent"></div>
          <div class="absolute bottom-4 left-4">
          <h3 class="text-white text-md font-bold">Manuel Hoyos</h3>
          <p class="text-primary font-medium text-xs">Desarrollador Full Stack</p>
          </div>
          </div>
          <div class="p-4 grow">
          <blockquote class="text-slate-600 dark:text-slate-400 italic text-sm border-l-2 border-primary pl-4">
              "La verdad es un mal necesario." - Solitario
          </blockquote>
          </div>
          <div class="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
          <div class="flex gap-3">
            <a class="text-slate-400 hover:text-primary transition-colors" href="#" target="_blank">
              <svg class="w-7 h-7 fill-current" viewbox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"></path></svg>  
            </a>
            <a class="text-slate-400 hover:text-primary transition-colors" href="#" target="_blank">
              <svg class="w-7 h-7 fill-current" viewbox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"></path></svg>
            </a>
          </div>
          <button class="bg-primary hover:bg-primary/90 text-slate-900 px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer">
              Connect
          </button>
          </div>
          </article>

                  <!-- Team Member 3 -->
          <article class="bg-white dark:bg-slate-900 border border-primary/10 rounded-xl overflow-hidden shadow-sm hover:shadow-md flex flex-col max-w-70 hover:scale-101 hover:-translate-y-2 transition-all group">
          <div class="h-50 w-full relative">
          <img alt="Cat sospechoso" class="w-full h-full object-cover" data-alt="Professional portrait of a male developer" src="./images/santiago-perfil/santiago-perfil.jpeg"/>
          <div class="absolute inset-0 bg-linear-to-t from-slate-900/70 to-transparent"></div>
          <div class="absolute bottom-4 left-4">
          <h3 class="text-white text-md font-bold">Santiago Cardoso</h3>
          <p class="text-orange-400 font-medium text-xs">Soporte servicio TI N2</p>
          </div>
          </div>
          <div class="p-4 grow">
          <blockquote class="text-slate-600 dark:text-slate-400 italic text-sm border-l-2 border-orange-400 pl-4">
              "La suprema excelencia consiste en romper la resistencia del enemigo." - Sun Tzu, el arte de la guerra
          </blockquote>
          </div>
          <div class="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
          <div class="flex gap-3">
            <a class="text-slate-400 hover:text-primary transition-colors" href="#" target="_blank">
              <svg class="w-7 h-7 fill-current" viewbox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"></path></svg>  
            </a>
            <a class="text-slate-400 hover:text-primary transition-colors" href="#" target="_blank">
              <svg class="w-7 h-7 fill-current" viewbox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"></path></svg>
            </a>
          </div>
          <button class="bg-orange-400 hover:bg-orange-400/90 text-slate-900 px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer">
              Connect
          </button>
          </div>
          </article>
        </div>
        </section>
      </div>
    `
})
export class AboutUsComponent { }