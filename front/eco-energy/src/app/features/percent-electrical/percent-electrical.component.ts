...
@Component({
  ...
  template: `
    <div class="animate-fade-in">
      <div class="mb-6">
        <h2 class="text-2xl font-black tracking-tight text-slate-900">% Renovable en Consumo Eléctrico</h2>
        <p class="text-slate-500">Porcentaje de energía renovable en el consumo eléctrico por región.</p>
      </div>
      ...
      <form [formGroup]="filterForm" (ngSubmit)="onSearch()" class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1" for="pe-year">Año</label>
          ...
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1" for="pe-limit">Límite</label>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-slate-100 bg-slate-50">
                ...
                <th class="px-6 py-3 text-right font-semibold text-slate-600">% Renovable</th>
...
export class PercentElectricalComponent implements OnInit {
  ...
  readonly chartOptions = computed(() => {
    ...
    return {
      ...
      dataLabels: { enabled: true, formatter: (val: number) => `${val}%`, style: { fontSize: '11px', fontWeight: 600 }, offsetY: -6 },
      xaxis: { categories: d.map((r) => r.region), labels: { style: { fontSize: '11px' }, rotate: -45, rotateAlways: true } },
      yaxis: { max: 100, labels: { style: { fontSize: '12px' }, formatter: (val: number) => `${val}%` } },
      fill: { opacity: 0.9 },
      tooltip: { y: { formatter: (val: number) => `${val}%` } },
      grid: { borderColor: '#f1f5f9', strokeDashArray: 4 },
      title: { text: `% Energía Renovable por Región (${d[0].year})`, style: { fontSize: '16px', fontWeight: '800', fontFamily: 'Inter', color: '#0f172a' } },
    };
  });
