...
@Component({
  ...
  template: `
    <div class="animate-fade-in">
      <div class="mb-6">
        <h2 class="text-2xl font-black tracking-tight text-slate-900">Participación Global en Electricidad</h2>
        <p class="text-slate-500">Distribución de fuentes renovables en el consumo eléctrico.</p>
      </div>
      ...
      <form [formGroup]="filterForm" (ngSubmit)="onSearch()" class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1" for="pa-entity">Entidad</label>
          ...
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1" for="pa-year">Año</label>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-x-auto">
            <div class="p-6 border-b border-slate-100">
              <h3 class="text-lg font-bold text-slate-900">Detalle de Participación</h3>
              <p class="text-sm text-slate-500">{{ data()[0]?.entity ?? '' }} — {{ data()[0]?.year ?? '' }}</p>
            </div>
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-slate-100 bg-slate-50">
                  <th class="px-6 py-3 text-left font-semibold text-slate-600">Fuente Energética</th>
                  <th class="px-6 py-3 text-right font-semibold text-slate-600">Participación</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr class="border-b border-slate-50 hover:bg-primary/5 transition-fast">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        ...
                        <span class="font-semibold text-slate-800">{{ formatSourceName(item.energySource) }}</span>
                      </div>
                    </td>
                    <td class="px-6 py-4 text-right">
                      <span class="font-bold text-slate-900 text-lg">{{ item.sharePercent | number:'1.2-2' }}%</span>
                    </td>
                  </tr>
                }
              </tbody>
            ...
            <div class="px-6 py-4 bg-slate-50 flex justify-between items-center border-t border-slate-200">
              <span class="font-bold text-slate-700">Total Combinado</span>
...
export class ParticipationComponent implements OnInit {
  ...
  readonly donutOptions = computed(() => {
    ...
    return {
      ...
      chart: { type: 'donut' as const, height: 420, fontFamily: 'Inter, sans-serif', animations: { enabled: true, easing: 'easeinout' as const, speed: 800 } },
      labels: d.map((item) => this.formatSourceName(item.energySource)),
      legend: { position: 'bottom' as const, fontSize: '13px', fontFamily: 'Inter', fontWeight: 500, markers: { size: 8 } },
        ...
        pie: {
          donut: {
            ...
            labels: {
              ...
              value: { show: true, fontSize: '22px', fontWeight: '800', fontFamily: 'Inter', formatter: (val: string) => `${val}%` },
              total: {
                show: true,
                label: 'Total',
                fontSize: '13px',
      ...
      title: { text: `Participación Eléctrica — ${d[0].entity} (${d[0].year})`, style: { fontSize: '16px', fontWeight: '800', fontFamily: 'Inter', color: '#0f172a' } },
    };
  });
